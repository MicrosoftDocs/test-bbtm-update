...
.SYNOPSIS
Checks-in localized RESX files resulted from the OneLocBuild task and seeds LCL files to a repo by creating and completing a pull request.

.DESCRIPTION
Checks-in localized RESX files generated by the preceding OneLocBuild task to a repo by creating and completing a pull request. Additionally, when
new files to localize are introduced, adds respective template LCL files to the payload of a pull request to commence localization in the downstream
...
.NOTES
In general, the OneLocBuild process is documented at https://aka.ms/onelocbuild.
The OneLocBuild task should be executed in advance but the 'Create a pull request to check-in localized files to the repo' option should be disabled
(on purpose) in the task to perform localized RESX file check-in and LCL file seeding at the same time using one single payload.
Both Git CLI and GitHub CLI should be available in order to make creating and completing a pull request work.
